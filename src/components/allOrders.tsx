...
const Order = () => {
    ...
    return (
        <section className='p-4 w-full flex flex-col gap-7 relative'>
            <h1 className='text-color1 font-semibold md:text-2xl text-xl'>My Orders</h1>
            <div className='bg-white rounded-md shadow-sm mb-12'>
                <div className='flex justify-around items-center p-5'>
                    {['All', 'New', 'Completed', 'Pending', 'Cancel'].map((tab, index) => (
                        <h1
                            ...
                            className={`cursor-pointer ${getUnderlineStyle(tab)} ${activeTab === tab ? 'active-tab text-color1 md:text-lg text-sm font-semibold' : 'inactive-tab text-gray-500 md:text-lg text-sm'}`}
                        >
                            {tab}
                        </h1>
                    ))}
                ...
                <div className='overflow-x-auto'>
                    <table className='min-w-full bg-white border border-gray-200'>
                        <thead>
                            <tr className='bg-color1 text-white md:text-[13px] text-sm'>
                            ...
                            {filteredOrders().slice(0, 5).map((order) => (
                                <tr key={order.id} className='border-b border-gray-200 text-base'>
                                    ...
                                    <td className='px-6 py-4'>
                                        ...
                                        <div className='flex items-center '>
                                            <Image src={order.image01} alt={order.productName} width={20} height={20} />
                                    ...
                                    <td className='px-6 py-4'>
                                        <Image src={order.image02} alt={order.productName} width={20} height={20} className='flex justify-center ml-[20px]'/>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
